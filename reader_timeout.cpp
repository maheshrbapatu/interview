...
void reader_function(int reader_id) {
    std::shared_lock<std::shared_timed_mutex> lock(resource_mutex, std::defer_lock);

    // Try to acquire the lock with a timeout of 1 second
    if (lock.try_lock_for(std::chrono::seconds(1))) {
        // If the lock is acquired within the timeout, proceed with reading
        std::cout << "Reader " << reader_id << " reads value as " << value << std::endl;
        // Shared lock is released when lock goes out of scope
    } else {
        // If the lock is not acquired within the timeout, handle the timeout case
        std::cout << "Reader " << reader_id << " could not read value due to timeout" << std::endl;
    }
}

void writer_function(int writer_id, int new_value) {
    std::unique_lock<std::shared_timed_mutex> lock(resource_mutex);
